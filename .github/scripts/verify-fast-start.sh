...
diagnostics() {
  ...
  if [[ -f compose.source-smoke.yaml ]]; then
    ...
    docker compose -f compose.source-smoke.yaml logs --no-color --tail=350 probo || true
  fi
  exit "${exit_code}"
}
